...
import "./App.css";
import { Container, Button, RowBox, Input, List, Todo,Wrap } from "./App.styled";
...
function App() {
  ...
  // 수정 버튼을 누른 뒤 나온 input의 값을 수정하는데에 써야하기 때문에 State에 담아 놓는다.
  const [editName, setEditName] = useState<string>('');

  // todoName stat를 input의 onChange에서 setState 해주는 함수
  ...
  // todo를 추가 해주는 함수
  const addTodo = () => {
    // todoName이 반값이면 추가하지 않음 (이후 코드 실행 X)
  ...
  // todos를 초기화 해주는 함수 (state, localStorage)
  const resetTodos = () => {
    setTodos([]);
  ...
  // 컴포넌트(페이지)가 "처음" 렌더링 됐을 ㄸ localStorage의 todos를 state에 넣어줌
  useEffect(() => {
  ...
  // state에 todos가 변화 될 때(추가, 삭제) localStorage에도 저장해줌
  useEffect(() => {
    ...
    <Container>
      ...
      <List>
        {todos.map(({ id, name }) => {
          ...
          const toggleEditTodo = () => setEditId(isEdit ? undefined : id);
          ...
          return (
            <Todo key={id}>
              {isEdit ? <input onChange={handleEditName} defaultValue={name} /> : name}
              <Button onClick={deleteTodo}>삭제</Button>

              <Button onClick={toggleEditTodo}>{isEdit ? '취소' : '수정'}</Button>
